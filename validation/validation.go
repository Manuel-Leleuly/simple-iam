...
func GetValidator() *validator.Validate {
	...
	// register alias
	validate.RegisterAlias("email", "required,email")
	...
	RegisterTranslation(validate, "username", "{0} must be alpha, min length 2, max length 10")
	RegisterTranslation(validate, "password", "{0} is required, min length 8")
	RegisterTranslation(validate, "name", "{0} must be alphanumeric, min length 5")
